...
export async function fetchBinanceData(symbol: string, forceFresh: boolean = false): Promise<MarketData | null> {
  try {
    ...
    // Retry-Logik für Netlify/Serverless-Umgebungen
      ...
      try {
        const response = await fetch(url, {
          ...
          headers: {
            ...
            'User-Agent': 'Mozilla/5.0 (compatible; Paragon/1.0)', // Wichtig für Netlify/Serverless
          },
          cache: 'no-store', // KEIN Cache - immer frische Daten
          // Entferne next: { revalidate: 0 } für Netlify-Kompatibilität
        });

        if (!response.ok) {
          const errorText = await response.text();
          console.error(`[fetchBinanceData] Binance API Error (attempt ${attempt}/${maxRetries}): ${response.status} ${response.statusText}`);
          ...
          if ((response.status === 429 || response.status >= 500) && attempt < maxRetries) {
            ...
            console.log(`[fetchBinanceData] Retrying after ${waitTime}ms...`);
        ...
        console.error(`[fetchBinanceData] Attempt ${attempt}/${maxRetries} failed:`, error);
    ...
    console.error(`[fetchBinanceData] All ${maxRetries} attempts failed. Last error:`, lastError);
    return null;
  } catch (error) {
    console.error(`[fetchBinanceData] Unexpected error:`, error);
    return null;
  }
}
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!response.ok) {
          const errorText = await response.text();
          console.error(`[fetchHistoricalData] API Error Response (attempt ${attempt}/${maxRetries}):`, errorText);
          
          // Retry bei 429 (Rate Limit) oder 5xx Fehlern
          if ((response.status === 429 || response.status >= 500) && attempt < maxRetries) {
            const waitTime = attempt * 1000; // Exponential backoff
            console.log(`[fetchHistoricalData] Retrying after ${waitTime}ms...`);
            await new Promise(resolve => setTimeout(resolve, waitTime));
            continue;
          }
        ...
        lastError = error instanceof Error ? error : new Error(String(error));
        console.error(`[fetchHistoricalData] Attempt ${attempt}/${maxRetries} failed:`, error);
    ...
    // Alle Versuche fehlgeschlagen
    console.error(`[fetchHistoricalData] All ${maxRetries} attempts failed. Last error:`, lastError);
    return null;
  } catch (error) {
    console.error(`[fetchHistoricalData] Unexpected error:`, error);
